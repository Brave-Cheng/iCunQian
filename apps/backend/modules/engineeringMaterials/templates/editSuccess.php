...
<div class="wrap">
    <?php echo form_tag("engineeringMaterials/update?" . html_entity_decode(formGetQuery("projectType", "keywords", "applicationId", "approvalId")), "id=updateMaterial") ?>
    ...
    <input type="hidden" name="engineeringMaterialsId" value="<?php echo $engineeringMaterials->getId();?>" />
        <div id="main" class="clearfix">
            <div class="full_width">
                <div class="bread_crumbs"> <a class="jump" href="<?php echo url_for('commitApproval/index');?>">审批流程</a> &gt; <span>修改审批</span> </div>
                <div class="formDiv pro_form data_display">
                    ...
                    <div class="left w300 ml20">
                        <div class="formItem">
                            ...
                            <span class="inblock">
                                <input type="text" name="contract_section" id="contract_section" class="txt" value="<?php echo $engineeringMaterials->getContractSection();?>" />
                    ...
                    <div class="right mr20">
                        <div class="formItem">
                            ...
                            <div class="iner">
                                <input type="text" name="number" id="number" class="txt" value="<?php echo $engineeringMaterials->getNumber();?>" />
                    ...
                    <div class="tables ex_tabs">
                        <table id="list">
                            ...
                            <?php foreach($engineeringMaterialsItems as $data):?>
                                <tr class="">
                                    <td></td>
                                    <td><input type="text" name="material_name[]" class="txt w70" value="<?php echo $data->getMaterialName();?>" /></td>
                                    <td><input type="text" name="brand[]" class="txt w60" value="<?php echo $data->getBrand();?>" /></td>
                                    <td><input type="text" name="technical_requirement[]" class="txt w50" value="<?php echo $data->getTechnicalRequireMent();?>" /></td>
                                    ...
                                    <td><input type="text" name="arrival_date[]" class="txt w70 timer" value="<?php echo $data->getArrivalDate();?>" /></td>
                                    <td><input type="text" name="arrival_place[]" class="txt w50" value="<?php echo $data->getArrivalPlace();?>" /></td>
...
$(function(){
    ...
    $(".jump").leaveCheck({formId:'updateMaterial',formUrl:'<?php echo url_for('engineeringMaterials/checkEngineeringMaterials?' . html_entity_decode(formGetQuery('projectId', 'pager'))); ?>'});
});
function initData(){
...
function checkForm(){
    ...
    /*
    if($.trim($('#contract_section').val()) != ''){
        var id = '<?php echo $engineeringMaterials->getId();?>';
        var obj = $.ajax({
            type:'get',
            url:'<?php echo url_for("engineeringMaterials/checkContractSection")?>',
            async: false,
            data:'contractSection='+ $('#contract_section').val()+'&id=' + id,
