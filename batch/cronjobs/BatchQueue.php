...
try {
    ...
    if (!in_array($type, ApiOfflineQueuePeer::getTypes())) {
    ...
    foreach ($str as $personalProduct) {
        $offlineQueue = new ApiOfflineQueue();
        $offlineQueue->setType($type);
        $offlineQueue->setBody(serialize($personalProduct));
        $offlineQueue->save();
    }
